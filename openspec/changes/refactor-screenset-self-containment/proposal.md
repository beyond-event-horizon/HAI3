...
1. **Auto-derive Redux slice names** - Slice name = screenset ID (e.g., `'chat'`)
2. **Auto-derive event namespaces** - Event prefix = `${screensetId}/` (e.g., `'chat/'`)
3. **Auto-derive API domains** - Domain = screenset ID (e.g., `'chat'`)
4. **Auto-derive translation namespaces** - Already done via `screenset.${id}:...`
5. **Auto-namespace icon IDs** - Prefix with screenset ID (e.g., `'chat:message-square'`)
6. **Auto-discover screensets** - Vite glob imports eliminate manual registration
7. **Standardize state keys** - RootState key = slice name (enforced by registerSlice)

**After Refactoring**: Duplicating chat → chat-copy requires:
1. Copy folder: `cp -r src/screensets/chat src/screensets/chat-copy`
2. Update screenset ID: `CHAT_SCREENSET_ID = 'chat'` → `'chat-copy'`
3. Update screen IDs in `screens/screenIds.ts`

**Total**: 3 steps, zero manual registration
...
### Benefits
- **99% reduction** in duplication effort (~50 steps → 3 steps)
...
- **Type safety** - Derived names prevent mismatches
