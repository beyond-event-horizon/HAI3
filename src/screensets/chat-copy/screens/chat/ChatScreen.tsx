...
const ChatScreenInternal: React.FC = () => {
  ...
  useScreenTranslations(CHAT_COPY_SCREENSET_ID, CHAT_COPY_SCREEN_ID, translations);
  ...
  useEffect(() => {
    void dispatch(chatCopyActions.fetchChatData());
  }, [dispatch]);
  ...
  const handleThreadSelect = useCallback((threadId: string) => {
    chatCopyActions.selectThread(threadId);
  }, []);

  const handleNewThread = useCallback(() => {
    const isTemporary = currentThread?.isTemporary || false;
    // Get localized "New chat" title
    const newChatTitle = t(`screenset.${CHAT_COPY_SCREENSET_ID}:new_chat`);
    // Create draft thread locally (no API call yet)
    // Thread will be created on backend when user sends first message
    chatCopyActions.createDraftThread(newChatTitle, isTemporary);
  }, [currentThread, t]);

  const handleDeleteThread = useCallback((threadId: string) => {
    void dispatch(chatCopyActions.deleteThread(threadId));
  }, [dispatch]);
  ...
  const handleSendMessage = useCallback(() => {
    if (!isStreaming && (inputValue.trim() || attachedFiles.length > 0) && currentThreadId) {
      ...
      const isTemporary = currentThread?.isTemporary || false;
      void dispatch(chatCopyActions.sendMessage(inputValue, currentThreadId, currentModel, conversationMessages, isTemporary));
    }
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    chatCopyActions.changeInputValue(value);
  }, []);

  const handleModelChange = useCallback((model: string) => {
    chatCopyActions.changeModel(model);
  }, []);

  const handleAddContext = useCallback((contextId: string) => {
    chatCopyActions.addContext(contextId);
  }, []);

  const handleRemoveContext = useCallback((contextId: string) => {
    chatCopyActions.removeContext(contextId);
  }, []);

  const handleFileSelect = useCallback((file: AttachedFile) => {
    chatCopyActions.attachFile(file);
  }, []);

  const handleFileRemove = useCallback((fileId: string) => {
    chatCopyActions.removeFile(fileId);
  }, []);
  ...
  const handleTitleSave = useCallback((newTitle: string) => {
    if (currentThreadId && newTitle.trim()) {
      void dispatch(chatCopyActions.updateThreadTitle(currentThreadId, newTitle.trim()));
    }
  ...
  const handleThreadTitleEdit = useCallback((threadId: string, newTitle: string) => {
    void dispatch(chatCopyActions.updateThreadTitle(threadId, newTitle));
  }, [dispatch]);

  const handleTemporaryToggle = useCallback((isTemporary: boolean) => {
    if (currentThreadId) {
      chatCopyActions.toggleThreadTemporary(currentThreadId, isTemporary);
    }
  }, [currentThreadId]);

  const handleThreadReorder = useCallback((newThreads: EnhancedChatThread[]) => {
    chatCopyActions.reorderThreads(newThreads);
  }, []);
  ...
  const handleEditMessage = useCallback((messageId: string) => {
    ...
    if (message) {
      chatCopyActions.startEditingMessage(messageId, message.content);
    }
  }, [currentMessages]);

  const handleEditedContentChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    chatCopyActions.updateEditedContent(e.target.value);
  }, []);

  const handleSaveEdit = useCallback(() => {
    if (editingMessageId && editedContent.trim()) {
      chatCopyActions.saveEditedMessage(editingMessageId, editedContent);
    }
  }, [editingMessageId, editedContent]);

  const handleCancelEdit = useCallback(() => {
    chatCopyActions.cancelEditingMessage();
  }, []);

  const handleLikeMessage = useCallback((messageId: string) => {
    chatCopyActions.likeMessage(messageId);
  }, []);

  const handleDislikeMessage = useCallback((messageId: string) => {
    chatCopyActions.dislikeMessage(messageId);
  }, []);
  ...
  const handleRegenerateMessage = useCallback((messageId: string) => {
    ...
    void dispatch(chatCopyActions.regenerateMessage(messageId, message.threadId, currentModel, conversationMessages));
  }, [currentMessages, currentModel, dispatch]);

  const handleDeleteMessage = useCallback((messageId: string) => {
    chatCopyActions.deleteMessage(messageId);
  }, []);

  const handleToggleViewMode = useCallback((messageId: string) => {
    chatCopyActions.toggleMessageViewMode(messageId);
  }, []);
