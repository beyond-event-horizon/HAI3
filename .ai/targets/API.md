...
## SCOPE
- Core API layer in `packages/uicore/src/api/**`
  - `plugins/` - Request/response interceptors (ApiPlugin, MockPlugin)
  - `protocols/` - Communication protocols (RestProtocol, SseProtocol)
  - `services/` - Domain services (accounts, etc.)
- App-level API extensions in `src/api/services/**`
...
## ADDING A SERVICE
1) Create `packages/uicore/src/api/services/<domain>/` with `ServiceNameApiService.ts` and `api.ts`
...
6) In app: add `src/api/services/<domain>/extra.ts` and `mocks.ts`
7) Import service in `src/api/apiRegistry.ts` to trigger registration
